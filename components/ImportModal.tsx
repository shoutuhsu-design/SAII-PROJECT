...
import { X, Upload, FileSpreadsheet, Loader2, CalendarCheck, Lock, Trash2, CheckSquare, Search, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 md:p-4">
      <div className="bg-white dark:bg-gray-800 md:rounded-lg shadow-xl w-full max-w-4xl flex flex-col h-full md:max-h-[90vh]">
        ...
        {/* Header - UPDATED: Increased top padding for mobile (from 48px to 64px) to ensure status bar clearance */}
        <div className="flex justify-between items-center px-4 pb-4 pt-[calc(env(safe-area-inset-top)+64px)] md:p-4 border-b dark:border-gray-700 bg-white dark:bg-gray-800 shrink-0">
          <div className="flex items-center gap-4">
              <h2 className="text-xl font-bold dark:text-white">{t.dataManagement}</h2>
              {/* Desktop Tabs */}
        ...
        <div className="md:hidden flex border-b dark:border-gray-700 p-2 gap-2 bg-gray-50 dark:bg-gray-900 overflow-x-auto">
            {isAdmin && (
                <>
                    <button onClick={() => setActiveTab('import')} className={`flex-1 py-2 text-xs font-bold rounded-lg whitespace-nowrap px-3 ${activeTab === 'import' ? 'bg-white shadow text-zte-blue' : 'text-gray-500'}`}>{t.importBtn}</button>
                    <button onClick={() => setActiveTab('batch')} className={`flex-1 py-2 text-xs font-bold rounded-lg whitespace-nowrap px-3 ${activeTab === 'batch' ? 'bg-white shadow text-zte-blue' : 'text-gray-500'}`}>{t.batchManage}</button>
                </>
            )}
            <button onClick={() => setActiveTab('export')} className={`flex-1 py-2 text-xs font-bold rounded-lg whitespace-nowrap px-3 ${activeTab === 'export' ? 'bg-white shadow text-zte-blue' : 'text-gray-500'}`}>{t.exportData}</button>
        </div>
        ...
        <div className="p-4 md:p-6 flex-1 overflow-y-auto">
          {activeTab === 'import' && (
              <div className="space-y-4 max-w-lg mx-auto">
                  ...
                  {/* Text Import Area (Hidden on mobile to simplify) */}
          ...
          {activeTab === 'export' && (
              <div className="flex flex-col items-center justify-start pt-12 md:pt-0 md:justify-center h-full gap-6">
                   <div className="text-center">
                       <h3 className="text-lg font-bold dark:text-white mb-2">{t.exportData}</h3>
                       <p className="text-gray-500 text-sm max-w-xs mx-auto">{t.exportDesc}</p>
                   </div>
                       ...
                       <table className="hidden md:table w-full text-sm text-left text-gray-500 dark:text-gray-400">
                           ...
                           <tbody>
                               {filteredTasks.length > 0 ? filteredTasks.map(task => {
                                   ...
                                   const u = users.find(u => u.employeeId === task.employeeId);
                                   return (
                                       <tr key={task.id} onClick={() => toggleSelection(String(task.id))} className={`cursor-pointer border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 ${isSelected ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}>
                                           <td className="p-3"><div className={`w-4 h-4 border rounded ${isSelected ? 'bg-zte-blue border-zte-blue' : 'border-gray-400'}`}/></td>
                       ...
                       <div className="md:hidden">
                            {filteredTasks.length > 0 ? filteredTasks.map(task => {
                                ...
                                return (
                                    <div 
                                        key={task.id} 
                                        onClick={() => toggleSelection(String(task.id))} 
                                        className={`p-4 border-b dark:border-gray-700 flex items-center gap-3 active:bg-gray-100 transition-colors ${isSelected ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
                                    >
                                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'border-zte-blue bg-zte-blue text-white' : 'border-gray-300'}`}>
                                            {isSelected && <CheckCircle size={14} />}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start">
                                                <h4 className="font-bold text-gray-900 dark:text-white text-sm truncate">{task.title}</h4>
                                                <span className={`px-2 py-0.5 rounded text-[10px] ${task.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>
                                                   {task.status}
                                               </span>
                                            </div>
