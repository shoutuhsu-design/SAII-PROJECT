...
const MainLayout: React.FC = () => {
  ...
  useEffect(() => {
    try {
      // 标记应用就绪，触发 index.html 中 app-shell 的淡出
      const signalReady = () => {
        document.body.classList.add('app-ready');
      };

      // 给予浏览器一小段渲染 React 树的时间
      ...
      console.error("Critical: Launch sequence failed", e);
      document.body.classList.add('app-ready'); // 兜底：即使报错也显示主页面
    }
  }, []);
  ...
  return (
    <div className={`flex flex-col h-screen-safe w-full bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-300 overflow-hidden`}>
        ...
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 h-14 sm:h-16 lg:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4 z-30 shrink-0">
             <div className="flex items-center gap-1 sm:border-r border-white/20 sm:pr-4">
                {user.role === 'admin' && (
                    <button onClick={() => setShowUserModal(true)} className="hidden md:block p-2.5 rounded-full hover:bg-white/20 text-white transition-colors relative group" title={t.userManagement}>
                        <Users size={20} />
                    </button>
                )}
                <button onClick={() => setShowImportModal(true)} className="hidden md:block p-2.5 rounded-full hover:bg-white/20 text-white transition-colors relative group" title={t.import}>
             ...
             <div className="hidden sm:flex items-center gap-2">
                <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="p-2 rounded-full hover:bg-white/20 text-white transition-colors">
                  {theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}
                </button>
                <button onClick={() => setLanguage(language === 'en' ? 'zh' : 'en')} className="w-8 h-8 flex items-center justify-center text-xs font-bold border border-white/30 rounded-full hover:bg-white/20 transition-colors">
                  {language === 'en' ? 'EN' : 'CN'}
                </button>
             </div>

             <button onClick={() => setShowMobileStats(true)} className="md:hidden p-2 rounded-full hover:bg-white/20 text-white transition-colors" title={t.stats}>
                <PieChart size={20} />
             </button>
             ...
             <div className="hidden lg:flex items-center gap-2 sm:gap-3 pl-1 sm:pl-2 sm:border-l border-white/20">
                ...
                <button onClick={logout} className="p-1.5 sm:p-2 rounded-full bg-white/10 hover:bg-red-500 hover:text-white transition-all border border-white/10 shadow-sm group" title={t.logout}>
            ...
            <div className="md:hidden absolute top-full left-0 right-0 bg-gradient-to-br from-[#E0F2FA] via-[#F0F9FF] to-white dark:from-gray-900/90 dark:via-gray-800/90 dark:to-gray-900/90 backdrop-blur-xl border-t border-white/40 dark:border-white/10 shadow-2xl z-50 animate-in slide-in-from-top-4 duration-200 safe-pb rounded-b-3xl mx-2 mt-1 overflow-hidden ring-1 ring-black/5 dark:ring-white/10">
                <div className="p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                         {user.role === 'admin' && (
                            <>
                                <button onClick={() => {setActiveTab('calendar'); setMobileMenuOpen(false)}} className={`col-span-2 p-3 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 border ${activeTab === 'calendar' ? 'bg-zte-blue text-white border-zte-blue shadow-md' : 'bg-white/60 dark:bg-white/5 text-gray-700 dark:text-gray-200 border-white/50 dark:border-white/5 shadow-sm'}`}><CalIcon size={24} /><span className="text-xs font-bold">{t.calendar}</span></button>
                                <button onClick={() => {setActiveTab('admin'); setMobileMenuOpen(false)}} className={`col-span-2 p-3 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 border ${activeTab === 'admin' ? 'bg-zte-blue text-white border-zte-blue shadow-md' : 'bg-white/60 dark:bg-white/5 text-gray-700 dark:text-gray-200 border-white/50 dark:border-white/5 shadow-sm'}`}><LayoutDashboard size={24} /><span className="text-xs font-bold">{t.dashboard}</span></button>
                            </>
                         )}
                         {user.role === 'admin' && (
                            <button onClick={() => {setShowUserModal(true); setMobileMenuOpen(false)}} className="aspect-square rounded-2xl bg-white/60 dark:bg-white/5 text-gray-700 dark:text-gray-200 flex flex-col items-center justify-center gap-1 border border-white/50 dark:border-white/5 shadow-sm"><Users size={20} /><span className="text-[10px] font-medium opacity-80">{t.userManagement.split(' ')[0]}</span></button>
                         )}
                         <button onClick={() => {setShowImportModal(true); setMobileMenuOpen(false)}} className="aspect-square rounded-2xl bg-white/60 dark:bg-white/5 text-gray-700 dark:text-gray-200 flex flex-col items-center justify-center gap-1 border border-white/50 dark:border-white/5 shadow-sm"><FileSpreadsheet size={20} /><span className="text-[10px] font-medium opacity-80">{t.import}</span></button>
                         <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="aspect-square rounded-2xl bg-white/60 dark:bg-white/5 text-gray-700 dark:text-gray-200 flex flex-col items-center justify-center gap-1 border border-white/50 dark:border-white/5 shadow-sm">{theme === 'light' ? <Moon size={20} /> : <Sun size={20} />}<span className="text-[10px] font-medium opacity-80">{theme === 'light' ? 'Dark' : 'Light'}</span></button>
                         <button onClick={() => setLanguage(language === 'en' ? 'zh' : 'en')} className="aspect-square rounded-2xl bg-white/60 dark:bg-white/5 text-gray-700 dark:text-gray-200 flex flex-col items-center justify-center gap-1 border border-white/50 dark:border-white/5 shadow-sm"><Globe size={20} /><span className="text-[10px] font-medium opacity-80">{language === 'en' ? 'CN' : 'EN'}</span></button>
                    </div>
                    <button onClick={logout} className="w-full bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-300 border border-red-100 dark:border-red-900/30 p-3 rounded-2xl flex items-center justify-center gap-2 font-bold shadow-sm"><LogOut size={18} /> {t.logout}</button>
                </div>
            </div>
        )}
      </header>
      ...
      <main className="flex-1 overflow-hidden flex max-w-[1920px] mx-auto w-full relative">
         <div className="hidden lg:block w-72 bg-white dark:bg-gray-800 border-r border-gray-100 dark:border-gray-700 flex-shrink-0 z-0 shadow-sm overflow-y-auto custom-scrollbar"><StatsSidebar /></div>
         <div className="flex-1 p-2 sm:p-4 lg:p-6 overflow-hidden relative h-full">{activeTab === 'calendar' ? <Calendar /> : <AdminDashboard />}</div>
         {showMobileStats && (
             <div className="fixed inset-0 z-40 lg:hidden">
                 ...
                 <div className="absolute left-0 top-0 bottom-0 w-80 bg-white dark:bg-gray-800 shadow-2xl z-50 flex flex-col animate-in slide-in-from-left duration-200 pt-[calc(env(safe-area-inset-top)+24px)] safe-pb">
                     <div className="flex items-center justify-between p-4 border-b dark:border-gray-700 mt-4">
                         ...
                         <button onClick={() => setShowMobileStats(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full"><X size={24} className="text-gray-500 dark:text-gray-400" /></button>
                     </div>
